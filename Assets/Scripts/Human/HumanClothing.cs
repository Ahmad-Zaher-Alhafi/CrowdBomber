...
public class HumanClothing : MonoBehaviour
{
    ...
    private void Start()
    {
        FindMeshRenderers(transform);
        CreateRandomClothing();
    }

    public void CreateRandomClothing()
    {
    ...
    public void FindMeshRenderers(Transform transform)
    {
        foreach (Transform t in transform)
        {
            Renderer rend;
            if (t.TryGetComponent<Renderer>(out rend))
            {
                renderers.Add(rend);
            }
    ...
    public void ApplyPoisonMaterial()
    {
